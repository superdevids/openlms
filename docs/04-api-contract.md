# REST API Contract — openlms Super-App (Single-School)
...
**Base URL:** `https://api.openlms.example/api/v1`
...
`SISWA, GURU, GURU_BK, KEUANGAN, OPERATOR, WAKEPSEK, KEPSEK, SUPERADMIN, CALON_SISWA, WALI_MURID, PEMBIMBING_INDUSTRI, PENGUJI_EKSTERNAL`

---

## 2. Endpoint per Modul

> Kolom Role memakai singkatan: S=SISWA, G=GURU, G_BK=GURU_BK, OPR=OPERATOR, K=KEUANGAN, WPS=WAKEPSEK, KPS=KEPSEK, SA=SUPERADMIN, CS=CALON_SISWA, WM=WALI_MURID, PI=PEMBIMBING_INDUSTRI, PE=PENGUJI_EKSTERNAL. `*` = semua role aktif. `P` = public (no auth). **homeroom (wali kelas) bukan role** — scope override untuk role GURU via `Class.homeroom_teacher_id` (prd04 §3.1).
...
// RES 201
{ "id": "usr_01H...", "status": "INVITED", "inviteLink": "https://app.openlms.id/invite/tok_..." }
```
...
```json
// RES 201
{ "token": "7X4K2M", "length": 6, "expiresAt": "2026-09-14T07:30:00.000Z",
...
| GET | `/attendance/records` | Rekap (filter student_id, date range) | S(own), G, G_BK, OPR, WPS, KPS, SA, WM(anak) |
| POST | `/attendance/permits` | Ajukan izin/sakit + upload surat (scope: diri sendiri; homeroom/wali untuk anak) | S, G, WM |
| PATCH | `/attendance/permits/:id` | Verifikasi izin | G(homeroom), G_BK, OPR, WPS, KPS, SA |
| GET | `/attendance/summary/:studentId` | Persentase kehadiran per mapel/semester | G(homeroom), G_BK, OPR, WPS, KPS, SA, WM |
...
| Method | Path | Deskripsi | Role |
|--------|------|-----------|------|
| GET/POST | `/counseling/notes` | Catatan BK (field-level: hanya GURU_BK/WAKEPSEK/KEPSEK) | G_BK, WPS, KPS, SA |
| GET/POST | `/discipline/points` | Katalog poin pelanggaran | OPR, WPS, SA |
| POST | `/discipline/records` | Catat pelanggaran siswa | G, G(homeroom), G_BK, OPR, WPS, KPS, SA |
| GET/POST | `/extracurriculars` (+ `/enrollments`) | Ekskul & pendaftaran | S, G, G_BK, OPR, WPS, KPS, SA |
...
| GET/POST | `/library/books` (+ `/loans`) | Katalog & peminjaman | S, G, G_BK, OPR, WPS, KPS, SA |
| GET/POST | `/announcements` | Pengumuman sekolah | * (buat: OPR, WPS, KPS, SA) |
| GET/POST | `/letters` (+ `/approve`) | Surat resmi & approval | S, G, G_BK, OPR, K, WPS, KPS, SA |
...
| GET/POST | `/internships` (+ `/journals`) | PKL & jurnal harian | S, PI, G(mentor), WPS, SA |
| GET/POST | `/internship-partners` (+ `/mentors`) | Mitra DUDI & pembimbing industri | WPS, SA |
| GET/POST | `/competency-tests` (+ rubric) | UKK & penilaian rubrik | PE, G, WPS, SA |
...
| GET | `/audit-logs` | Riwayat audit (filter entity) | WPS, KPS, SA |
...
| Ujian: kerjakan | ✓ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ |
| Ujian: nilai esai/analisis | ✗ | ✓ | ✗ | ✗ | ✗ | ✗ | △ (baca) | ✗ | ✓ | ✗ | ✗ | ✗ |
| Ujian: log aktivitas | ✗ | ✓ | ✗ | ✗ | ✗ | ✗ | ✓ | ✗ | ✓ | ✗ | ✗ | ✗ |
...
| Ekskul: daftar | ✓ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ |
| Kepegawaian: data staf | ✗ | △ (diri) | ✗ | △ (diri) | ✓ | △ | ✓ | ✓ | ✓ | ✗ | ✗ | ✗ |
| Absensi staf | ✗ | △ | ✗ | △ (diri) | ✓ | △ | △ | ✓ | ✓ | ✗ | △ | ✗ |
| Sarpras: kelola aset | ✗ | ✗ | ✗ | ✗ | ✓ | ✗ | ✓ | ✓ | ✓ | ✗ | ✗ | ✗ |
| Sarpras: booking | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✗ | ✗ | ✗ |
| Perpustakaan: pinjam | ✓ | ✓ | ✓ | ✓ | ✓ | ✗ | ✓ | ✓ | ✓ | ✗ | ✗ | ✗ |
| Komunikasi: pengumuman | △ (baca) | △ | △ (baca) | △ (baca) | ✓ | △ | ✓ | ✓ | ✓ | ✗ | △ | △ |
| Surat resmi: ajukan/approve | △ (ajukan) | △ | ✗ | △ (ajukan) | ✓ | △ | ✓ | ✓ | ✓ | ✗ | ✗ | ✗ |
...
> Catatan: semua akses "✓/△" tetap dibatasi **scope RBAC (SENDIRI/KELAS/SEKOLAH)**; △ berarti scope milik sendiri/kelasnya/anaknya atau memerlukan approval; **homeroom (wali kelas) = scope override untuk role GURU** via `Class.homeroom_teacher_id`; **SUPERADMIN = admin sistem sekolah** (bukan penyedia SaaS) — seluruh data milik SATU sekolah dan dicatat di `AuditLog`.
