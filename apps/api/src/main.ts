...
/**
 * CORS_ORIGINS (koma-pisah) → array origin.
 * Di production, CORS_ORIGINS WAJIB diset — fallback localhost dev TIDAK dipakai
 * (fail-fast saat boot, sama seperti JWT secrets di jwt.util).
 */
function allowedOrigins(): string[] {
  ...
  if (!raw || raw.trim().length === 0) {
    if (isProduction) {
      throw new Error(
        "[main] CORS_ORIGINS wajib dikonfigurasi di production (fail-fast). " +
